...
namespace pluto
{
    ...
    FontAssetCompiler::FontAssetCompiler(FileManager& fileManager)
    ...
    std::unique_ptr<FontAsset> FontAssetCompiler::Compile(const Path& path)
    {
        ...
        stbtt_BakeFontBitmap(bytes.data(), 0, fontSize, bitmap.data(), bitmapWidth, bitmapHeight, first,
                             bakedChars.size(),
                             bakedChars.data());
        ...
        std::unique_ptr<FontAsset> fontAsset = fontAssetFactory->Create(fontSize, glyphs, bitmapWidth, bitmapHeight,
                                                                        bitmap);
        return fontAsset;
    }
}
